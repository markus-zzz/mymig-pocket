...
public:
  ...
  void Tick() {
    ...
    if (dut->video_hs) {
      m_HCntr = 0;
      if (m_DisplayEnableSinceVSync)
        m_VCntr++;
    }
    if (dut->video_vs) {
      m_VCntr = 0;
      m_DisplayEnableSinceVSync = false;
      FrameDone = true;
    }

    if (dut->video_de) {
      if (0 <= m_HCntr && m_HCntr < c_Xres && 0 <= m_VCntr &&
          m_VCntr < c_Yres) {
        guchar Red = dut->video_rgb >> 16;
        guchar Green = dut->video_rgb >> 8;
        guchar Blue = dut->video_rgb;
        PutPixel(m_FramePixBuf, m_HCntr, m_VCntr, Red, Green, Blue);
      }

      m_DisplayEnableSinceVSync = true;
      m_HCntr++;
    }

    if (FrameDone) {
      char buf[32];
      snprintf(buf, sizeof(buf), "video-%04d.png", g_frame_idx);
...
private:
  ...
  const unsigned c_Yres = 256;
  GdkPixbuf *m_FramePixBuf;
  unsigned m_HCntr = 0;
  unsigned m_VCntr = 0;
  bool m_DisplayEnableSinceVSync;
};
